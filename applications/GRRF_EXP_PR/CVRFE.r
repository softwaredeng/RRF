...
RFE=varSelRF(trainX, as.factor(trainY), c.sd = 1, mtryFactor = 1, ntree = ntree,
...
RFEFS=RFE$selected.vars;#RFEFS=as.numeric(gsub("v","",RFEFS))

trainX2=trainX[,RFEFS,drop=FALSE]
testX2=testX[,RFEFS,drop=FALSE]

resF=testF(trainX2,trainY,testX2,testY,ntree)
err=c(err,resF$errThis);
colN=c(colN,paste("sample",foldI,"_",thisData,"_",resF$colNThis,sep=""))

fN=c(fN,matrix(length(RFEFS),1,length(resF$errThis)));
...
timeStamp=paste("RFE","_rep",nRep,"_ntree",ntree,"_",timeStamp,".RData",sep="");
...
all = cbind(err,fN)
rownames(all)=colN
pathOut=paste(getwd(), "/result/","Rep",nRep,"RFE",sep="");
