...
set.seed(1)
source('sub_classifier.r');library(RWeka);
...
ptm <- proc.time()
 ...
 lambdaV = c(0.01,0.02,0.03,0.05,0.1,0.2)
# build the model; due to large data, 5-fold CV is used to select the best lambda; More tuning/parameters can be used for further improvements

cvNet <- try(  cv.glmnet(trainX,as.factor(trainY),lambda=lambdaV,nfolds=nFold,family="multinomial")   )
if(isTRUE(all.equal(class(cvNet), "try-error"))) {
    writeLines("Model failed")
err=c(err, NA_integer_,NA_integer_,NA_integer_);
fN= c(fN,  NA_integer_,NA_integer_,NA_integer_);
colN=c(colN,paste(thisData,"_","sample",foldI,"_","LASSO",sep=""))
colN=c(colN,paste(thisData,"_","sample",foldI,"_","RF",sep=""))
colN=c(colN,paste(thisData,"_","sample",foldI,"_","Tree",sep=""))

} else { #----
#cvNet=cv.glmnet(trainX,as.factor(trainY),lambda=lambdaV,nfolds=nFold,family="multinomial")
# coefficients of the model
coef = coef(cvNet);dimCoef = nrow(summary(coef))
ixLasso = NULL
for(dimI in 1:dimCoef)
{
 coefTmp=as.double((coef[[dimI]]))
 coefTmp=coefTmp[-1]#delete the intercept coefficient
 ixLasso = c(ixLasso,which(coefTmp!=0))
}
ixLasso = unique(ixLasso)
fnLasso = length(ixLasso)

# use LASSO
pred=predict(cvNet,newx=testX,type="class")
pred=as.character(pred)
errLASSO=1-sum(pred==testY)/length(pred);
...
trainX2=trainX[,ixLasso,drop=FALSE]
testX2=testX[,ixLasso,drop=FALSE]

resF=testF(trainX2,trainY,testX2,testY,ntree)
err=c(err,resF$errThis);
colN=c(colN,paste(thisData,"_","sample",foldI,"_",resF$colNThis,sep=""))

fN=c(fN,matrix(length(ixLasso),1,length(resF$errThis)));
} #----else  
...
timeStamp=paste("LASSO_","_rep",nRep,"_",timeStamp, ".RData",sep="");
...
all = cbind(err,fN)
rownames(all)=colN
pathOut=paste(getwd(), "/result/","Rep",nRep,"LASSO",sep="");
